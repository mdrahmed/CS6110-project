...
static void TEST_framing_connack(){
    uint32_t buf[] = {
        (MQTT_CONTROL_CONNACK << 4) | 0,2,0, MQTT_CONNACK_ACCEPTED
    };
    struct mqtt_response mqtt_response;
    ssize_t rv = mqtt_fixed_header(mqtt_response, buf, sizeof(buf));

    if (rv == 2) {
        printf("rv is correct");
    }
    else {
        prinf("rv is wrong");
    }
